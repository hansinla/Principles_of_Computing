...
def strategy(hand, num_die_sides):
    ...
    for held_dice in all_hands:
        num_free_dice = len(hand) - len(held_dice)
        score = expected_value(held_dice, num_die_sides, num_free_dice)
        if score > max_score_hold[0]:
            max_score_hold = (score, held_dice)
    
    return max_score_hold


def run_example():
    """
    Compute the dice to hold and expeced score for an example hand
    """
    num_die_sides = 6
    hand = (1, 1, 1, 5, 6)
